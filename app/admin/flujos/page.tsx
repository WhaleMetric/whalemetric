...
function FlowCard({ flow, onToggle, onChangeInterval, onExecute, executing }: FlowCardProps) {
  ...
  async function handleToggle() {
    setToggling(true);
    try { await onToggle(flow.flow_key, !flow.enabled); }
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5 flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          <div className={`mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${flow.enabled ? 'bg-emerald-50' : 'bg-gray-100'}`}>
            <ArrowPathIcon className={`w-4 h-4 ${flow.enabled ? 'text-emerald-600' : 'text-gray-400'}`} />
          </div>
      ...
      <div className="flex items-center gap-4 flex-wrap text-xs text-gray-500">
        <span className="flex items-center gap-1.5">
          ...
          Último: <span className="text-gray-700 font-medium ml-0.5">{timeAgo(flow.last_run_at)}</span>
        </span>
        <span className="flex items-center gap-1.5">
          ...
          {editingInterval ? (
            ...
            <button
              ...
              className="text-gray-700 font-medium underline underline-offset-2 decoration-dashed hover:text-gray-900 transition-colors ml-0.5"
            >
      ...
      <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
        ...
        >
          {executing ? <ArrowPathIcon className="w-3.5 h-3.5 animate-spin" /> : <BoltIcon className="w-3.5 h-3.5" />}
          Ejecutar ahora
        </button>
        {!flow.enabled && (
          <span className="text-xs text-gray-400 italic">Activa el flujo para ejecutarlo</span>
        )}
      </div>
    </div>
  );
...
export default function FlujosPage() {
  ...
  async function handleToggle(flow_key: string, enabled: boolean) {
    ...
    setFlows((prev) => prev.map((f) => f.flow_key === flow_key ? { ...f, enabled } : f));
  ...
  async function handleChangeInterval(flow_key: string, interval_minutes: number) {
    ...
    const json = await res.json();
    if (!res.ok) { showToast(json.error ?? 'Error', false); return; }
    setFlows((prev) => prev.map((f) => f.flow_key === flow_key ? { ...f, interval_minutes } : f));
  ...
  async function handleExecute(flow_key: string) {
    ...
    try {
      ...
      setFlows((prev) => prev.map((f) => f.flow_key === flow_key ? { ...f, last_run_at: new Date().toISOString() } : f));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium ${toast.ok ? 'bg-gray-900 text-white' : 'bg-red-50 text-red-700 border border-red-200'}`}>
          {toast.ok
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <a href="/admin" className="flex items-center gap-1 text-gray-400 hover:text-gray-700 text-sm transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <div className="text-gray-400 text-xs mt-1 max-w-xs">
              ...
              <code className="bg-gray-100 px-1 rounded">automation_flows</code> de Supabase para verlos aquí.
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {!loading && flows.length > 0 && (
          <div className="mt-8 flex items-start gap-2 text-xs text-gray-400 bg-gray-100 rounded-xl px-4 py-3">
            ...
            <span>
              Los intervalos son orientativos — la ejecución real depende del scheduler de Supabase Edge Functions o el cron externo.{' '}
              <a href="https://supabase.com/dashboard/project/txxygcdafjcuyvvzbbnx" target="_blank" rel="noopener noreferrer" className="underline hover:text-gray-600 transition-colors">
                Ver Supabase →
              </a>
            </span>
          </div>
